...
|--------|---------|------|---------|-----------------|
| `sendafun_pay_per_send` | 按次发送 | $1.99/次 | 一次性 | 待创建 |
| `sendafun_monthly_subscription` | 月订阅 | $6.99/月 | 月付，自动续费 | 待创建 |
| `sendafun_annual_subscription` | 月订阅年付 | $69/年 | 年付，自动续费 | 待创建 |
...
| Price ID (单张) | `price_xxxxx` ← 需在 Creem 后台创建后替换 |
| Price ID (年费) | `price_yyyyy` ← 需在 Creem 后台创建后替换 |
| Price ID (回送首次) | `price_zzzzz` ← $0.99，需在 Creem 后台创建 |

> 代码中 `PRICE_SINGLE = "price_single_card"` 和 `PRICE_ANNUAL = "price_annual_sub"` 是占位符，必须替换为真实 Creem Price ID。
