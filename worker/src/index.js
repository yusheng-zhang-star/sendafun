 ...
 */class RateLimiter {
  ...
  async check(k){
    ...
    const t=Math.max(60,Math.ceil((ws+6e4-n)/1e3)+10);
...
const CREEM_PRODUCTS = {
  single:   "prod_7GGx4Gh5yvKLOb0OCzYFoq",  // $1.99 Pay Per Send (one-time)
  monthly:  "prod_3xVdtK0wdzqLlaCz4H7lzQ",  // $6.99 Monthly Unlimited (monthly recurring)
  annual:   "prod_73aCoww3uhNMevKi8NVwNv"   // $69 Annual Unlimited (yearly recurring)
};

// Plan → TTL seconds for permission grant
const PLAN_TTL = { single: 86400, monthly: 2592000, annual: 31536000 };

async function createCreemSession(productId,email,succUrl,cancUrl,meta){
  const body={
...
export default{
  async fetch(request,env){
    // ── Security guidance ──────────────────────────────────────────────
    // PRODUCTION: ALWAYS set CREEM_API_KEY / CREEM_WEBHOOK_SECRET /
    // RESEND_API_KEY as Cloudflare Worker Secrets via:
    //   wrangler secret put CREEM_API_KEY
    //   wrangler secret put CREEM_WEBHOOK_SECRET
    //   wrangler secret put RESEND_API_KEY
    // Never commit real secrets to source control.
    //
    // LOCAL DEV fallback (only used when env.CREEM_API_KEY is undefined):
    const CREEM_DEV_TEST_KEY = "creem_test_7deQTeY7iE1fapgeaiLQ1u";
    // ───────────────────────────────────────────────────────────────────
    CREEM_API_KEY=env.CREEM_API_KEY||CREEM_DEV_TEST_KEY;
    CREEM_WEBHOOK_SECRET=env.CREEM_WEBHOOK_SECRET||"";
    SITE_URL=env.SITE_URL||"https://sendafun.com";
    RESEND_API_KEY=env.RESEND_API_KEY||"";
    RESEND_FROM_EMAIL=env.RESEND_FROM_EMAIL||"onboard@resend.dev";
